...
echo "deb [trusted=yes] https://raw.githubusercontent.com/marjosvilAA/debianpackagerepository/main/ ./" > /etc/apt/sources.list.d/globaleaks.list
...
while true; do
    read -p "Do you wish to enter your OpenAI API key? [y|n] " yn
    case $yn in
        [Yy]*)
            read -p "Please enter your OpenAI API key: " api_key
            # Save the API key to /etc/environment
            sudo sh -c "echo 'OPENAI_API_KEY=\"$api_key\"' >> /etc/environment"
            echo "API key saved to /etc/environment."
            break
            ;;
        [Nn]*)
